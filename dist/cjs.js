...
function getRoutineDump(connection, dbName, options) {
    return __awaiter(this, void 0, void 0, function* () {
        const format$$1 = (sql) => sqlformatter.format(sql);
        ...
        for (const routine of routines) {
            try {
                ...
                if (createResult.length > 0 &&
                    ...
                    let sql = createResult[0]['Create Function'] ||
                        createResult[0]['Create Procedure'];
                    // Clean up the generated SQL
                    ...
                    if (sql && sql.trim()) {
                        ...
                        console.warn(`⚠️  Rotina ${routine.ROUTINE_NAME} não possui SQL válido, pulando...`);
                    }
                }
            }
            catch (error) {
...
function buildInsert(table, values, format$$1, selectedColumns) {
...
function buildInsertValue(row, table, selectedColumns) {
    const columns = selectedColumns || table.columnsOrdered;
        ...
        try {
            ...
            while (tables.length > 0) {
                ...
                yield new Promise((resolve, reject) => {
                    ...
                    const selectedColumns = options.columns && options.columns[table.name]
                        ? options.columns[table.name]
                        : undefined;
                    const columnsToSelect = selectedColumns
                        ? `\`${selectedColumns.join('`,`')}\``
                        : '*';
                    const query = connection.query(`SELECT ${columnsToSelect} FROM \`${table.name}\`${where}`);
                    let rowQueue = [];
                    // stream the data to the file
                    query.on('result', (row) => {
                        // build the values list
                        rowQueue.push(buildInsertValue(row, table, selectedColumns));
                        // if we've got a full queue
                        if (rowQueue.length === options.maxRowsPerInsertStatement) {
                            // create and write a fresh statement
                            const insert = buildInsert(table, rowQueue, format$$1, selectedColumns);
                            saveChunk(insert);
                            rowQueue = [];
                        }
                    });
        ...
        try {
            ...
            if (options.dump.routine !== false) {
                ...
                try {
                    ...
                    routineConnection = yield DB.connect(deepmerge.all([options.connection, { multipleStatements: true }]));
